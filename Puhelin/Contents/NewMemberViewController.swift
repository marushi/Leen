...
class NewMemberViewController: UIViewController,UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout {
    ...
    func setUp(){
        ...
        var Ref:Query!
        Ref = Firestore.firestore().collection(UserDefaultsData.init().opDB!).limit(to: limitNum!).whereField("searchPermis", isEqualTo: true).order(by: "signupDate",descending: true)
                    ...
                    for i in 0...num2{
                        ...
                        ref.getDocument(){(data,error) in
                            ...
                            if num == self.setUserDataArray.count {
                                let sortedBysignupDate: SortDescriptor<MyProfileData> = { $0.signupDate!.dateValue() > $1.signupDate!.dateValue() }
                                let result = self.setUserDataArray.sorted(by: sortedBysignupDate)
                                self.setUserDataArray = result
                                self.refreshControl.endRefreshing()
                                //リフレッシュ終了
    ...
    func setUp2(){
        ...
        //並び替え分岐
        var Ref:Query?
        Ref = Firestore.firestore().collection(UserDefaultsData.init().opDB!).limit(to: limitNum!).whereField("searchPermis", isEqualTo: true).order(by: "signupDate",descending: true).start(afterDocument: last!)
        Ref!.getDocuments() { (querySnapshot, error) in
        if let error = error {
            print("DEBUG_PRINT: snapshotの取得が失敗しました。 \(error)")
            self.collectionView.hideSkeleton()
