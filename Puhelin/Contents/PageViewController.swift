...
class PageViewController: UIPageViewController, UIPageViewControllerDataSource, UIPageViewControllerDelegate, UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
    ...
    static let collectionViewHeight = App.statusBarHeight + 70
    private let cellWidth: CGFloat = 108
    
    // ~~~~~~~~~Page側~~~~~~~~~
    private let pagelist: [String] = ["おすすめ", "ログイン順", "新ユーザー","条件でさがす"]
    private (set) var pageControllergrop = [UIViewController]()
    // 初回のPage番号、現在のPage番号
    static let firstPageNum: NSInteger = 0
    private (set) var selectedPageNum:Int! = 0
    // スクロール開始位置
    ...
    private var selectBarBackView = UIView()
    // 現在のセル番号
    ...
    override func viewDidLoad() {
        ...
        self.view.backgroundColor = ColorData.snow
        // 1
        let goodView = UIView(frame: CGRect(x: 0, y: 0,
                            width: 100, height: 30))
        goodView.backgroundColor = .clear
         
        // 2
        let item = UIBarButtonItem(customView: goodView)
        self.navigationItem.leftBarButtonItem = item
        
        let imageView = UIImageView(image: UIImage(systemName: "hand.thumbsup"))
        imageView.frame = CGRect(x: 7, y: 5, width: 25, height: 25)
        imageView.tintColor = .darkGray
        goodView.addSubview(imageView)
        
        label.frame = CGRect(x: 40, y: 5, width: 30, height: 25)
        label.text = "10"
        label.textColor = .darkGray
        goodView.addSubview(label)
    }
    ...
    func setViewControllers() {
        ...
        let view4 = self.storyboard?.instantiateViewController(identifier: "Search") as! Search
        self.pageControllergrop = [view1,view2,view3,view4]
        // 高速スクロールを封じるViewをセット
    ...
    func setCollectionView() {
        ...
        self.collectionView.backgroundColor = ColorData.snow
    ...
    func setSelectBarView() {
        self.selectBarView.backgroundColor = ColorData.darkturquoise
        self.selectBarView.frame.size = CGSize(width: self.cellWidth,  height: 1)
        ...
        self.selectBarBackView.backgroundColor = ColorData.gainsboro
        self.selectBarBackView.frame.size = CGSize(width: App.windowWidth, height: 1)
        self.selectBarBackView.center.y = PageViewController.collectionViewHeight - 1
        
        self.view.addSubview(self.selectBarBackView)
    ...
    private func nextViewController(viewController: UIViewController, isAfter: Bool) -> UIViewController? {
        guard var index = self.pageControllergrop.firstIndex(of: viewController) else { return nil }
    ...
    private func changeSelectBar(_ nextPageNum: NSInteger) {
        ...
        UIView.animate(
            withDuration: 0.25,
            animations: {
                self.selectBarView.frame.size = CGSize(width: titleWidth,  height: 1)
    ...
    func pageViewController(_ pageViewController: UIPageViewController, didFinishAnimating finished: Bool, previousViewControllers: [UIViewController], transitionCompleted completed: Bool){
        ...
        if let viewController = pageViewController.viewControllers?.first {
            guard let index = pageControllergrop.firstIndex(of: viewController) else { return }
