...
class LoginOrderViewController: UIViewController,UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout {
    ...
    func setUp(){
        ...
        var Ref:Query!
        Ref = Firestore.firestore().collection(UserDefaultsData.init().opDB!).limit(to: limitNum!).whereField("searchPermis", isEqualTo: true).order(by: "LoginDate",descending: true)
                    ...
                    for i in 0...num2{
                        ...
                        ref.getDocument(){(data,error) in
                            ...
                            if num == self.setUserDataArray.count {
                                let sortedByLoginDate: SortDescriptor<MyProfileData> = { $0.loginDate!.dateValue() > $1.loginDate!.dateValue() }
                                let result = self.setUserDataArray.sorted(by: sortedByLoginDate)
    ...
    func setUp2(){
        ...
        //並び替え分岐
        var Ref:Query?
        Ref = Firestore.firestore().collection(UserDefaultsData.init().opDB!).limit(to: limitNum!).whereField("searchPermis", isEqualTo: true).order(by: "LoginDate",descending:  true).start(afterDocument: last!)
        Ref!.getDocuments() { (querySnapshot, error) in
        if let error = error {
            print("DEBUG_PRINT: snapshotの取得が失敗しました。 \(error)")
            self.collectionView.hideSkeleton()
